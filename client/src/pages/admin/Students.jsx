...
const handleLogout = () => {
  // 1. LocalStorage se user ka data delete karo
  localStorage.removeItem("user");

  // 2. Login page par wapas bhej do
  window.location.href = "/login";
};

const Students = () => {
  // Mock Data (Abhi ke liye)
  ...
  return (
    <div className="animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <button className="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-3 rounded-xl flex items-center gap-2 shadow-lg shadow-indigo-200 transition-all font-bold active:scale-95 w-full md:w-auto justify-center">
          <UserPlus size={20} />
          Add New Student
        </button>
        <button
          onClick={handleLogout} // <--- Yahan use ho gaya handleLogout
          className="flex items-center gap-2 bg-red-50 text-red-600 px-4 py-2 rounded-xl font-bold hover:bg-red-600 hover:text-white transition-all border border-red-100"
        >
          <LogOut size={18} />
          <span>Logout</span>
        </button>
